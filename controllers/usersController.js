...
class usersController {
  ...
  async areFrens(userid1, userid2){
    const q = await pool.query(`select * from contactlist where userid=$2 and userfriend=$1;`,[userid1,userid2]);
    return q.rows;
  }

  async addTrade(uid1,uid2,c1,c2,date){
    const q=await pool.query(`insert into trades values ($1,$2,$3,$4,$5)`,[uid1,uid2,c1,c2,date]);
    if (q.err) return { error: q.err };
    return q;
  }

  async getTradesForUser(uid){
    const q=await pool.query(`select u1.username, u1.userid, to_json(c1) card1, to_json(c2) card2, t."date" from users u1, users u2, cards c1, cards c2, trades t where t.user1=u1.userid and t.user2=u2.userid and t.card1=c1.cardid and t.card2=c2.cardid and t.user2=$1;`,[uid]);
    if (q.err) return { error: q.err };
    return q.rows;
  }

  async acceptTrade(uid1,uid2,c1,c2,date){
    const t1 = await pool.query(`with cte as (select id from usercards where cardid = $1 and userid = $2 limit 1) update usercards u set userid = $3 from cte where u.id = cte.id returning u.id;`,[c1,uid1,uid2]);
    let t2;
    if(t1.rows.length>0) t2 = await pool.query(`with cte as (select id from usercards where cardid = $1 and userid = $2 limit 1) update usercards u set userid = $3 from cte where u.id = cte.id returning u.id;`,[c2,uid2,uid1]);
    const d = await pool.query(`delete from trades where user1=$1 and user2=$2 and card1=$3 and card2=$4 and date=$5;`,[uid1,uid2,c1,c2,date]);
